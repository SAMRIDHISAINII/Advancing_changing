...
from app.unet.augmentation import *
...
def load_images_predict(before, after, batch_size=1):
    inputs = np.zeros(shape=(1, 6, 650, 650))
    masks = np.zeros(shape=(1, 650, 650), dtype=np.long)
    img_m = np.zeros(shape=(650, 650))
    input, mask = images_prepare(before, after, img_m)
    inputs[0] = input
    masks[0] = mask
    inputs = inputs[:1]
    print(inputs.shape)
...
def draw_mask(after, mask):
    after = np.asarray(after)
    mask = cv2.cvtColor(np.array(mask), cv2.COLOR_RGB2BGR)
    dst = cv2.addWeighted(after, 0.8, mask, 0.2, 4)
    visualized_res = cv2.cvtColor(dst, cv2.COLOR_BGR2RGB)
    im_pil = Image.fromarray(visualized_res)
    return im_pil
...
def make_mask(after_image, mask):
    after_image = cv2.cvtColor(after_image, cv2.COLOR_RGBA2BGR)
    print(after_image.shape)
    mask = cv2.cvtColor(np.array(mask), cv2.COLOR_RGB2BGR)
    cv2.imshow("re", mask)
    cv2.waitKey(0)
    masked_data = cv2.bitwise_and(after_image, after_image, mask=mask)
